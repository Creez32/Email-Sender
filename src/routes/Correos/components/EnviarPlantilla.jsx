...
export default function EnviarPlantilla() {
    ...
    const handleAceptar = async () => {
        if (!cuil.trim()) {
            Swal.fire({
                title: 'Error',
                text: 'Por favor ingresa un CUIL válido.',
                icon: 'error',
        ...
        try {
            ...
            if (!res || !res.data) {
                Swal.fire({
                    title: 'Error',
                    text: 'No se encontraron datos para el CUIL ingresado.',
                    icon: 'error',
                    confirmButtonText: 'Aceptar',
                });
                return;
            ...
            Swal.fire({
                title: 'Error',
                text: 'Error al buscar afiliado o plantillas.',
                icon: 'error',
                confirmButtonText: 'Aceptar',
            });
        }
    ...
    const handleEnviar = async () => {
        if (!plantillaSeleccionada) {
            Swal.fire({
                title: 'Error',
                text: 'Selecciona una plantilla primero.',
                icon: 'error',
                confirmButtonText: 'Aceptar',
            });
            return;
        ...
        try {
            const payload = {
                ...
                afiliado: afiliado.CUILAfiliado,
            };
                ...
                <div className="step-plantillas">
                    ...
                    <div className="lista-plantillas">
                        {plantillas.map((p) => (
                            <div
                                key={p.id}
                                className={`card-plantilla ${plantillaSeleccionada?.id === p.id ? "selected" : ""
                                    }`}
